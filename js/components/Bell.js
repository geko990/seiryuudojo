...
export default class Bell {
    ...
    async render() {
        ...
        // Fetch notifications from Sheet
        await this.fetchNotifications();
        const unreadCount = this.getUnreadNotifications().length;

        // Bronze bell SVG icon with badge
        container.innerHTML = `
           ...
           ${unreadCount > 0 ? `<span class="notify-badge" style="position: absolute; top: -5px; right: -5px; background: #f44336; color: white; font-size: 10px; font-weight: bold; padding: 2px 6px; border-radius: 10px; min-width: 14px; text-align: center;">${unreadCount}</span>` : ''}
        `;
